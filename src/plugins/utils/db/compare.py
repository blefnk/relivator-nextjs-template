...
def parse_schema(file_content):
    ...
    schema = {}
    table_defs = re.findall(r'export const (\w+) = [mp]gTable\("(\w+)", \{([\s\S]*?)\}\);', file_content)
    for table_def in table_defs:
        table_name = table_def[1]
        fields = re.findall(r'(\w+): (\w+)\("(\w+)"(?:, \{([\s\S]*?)\})?\)', table_def[2])
        schema[table_name] = {field[0]: {'type': field[1], 'options': field[3]} for field in fields}
    return schema
    ...
    for table in schema1:
        ...
        for field in schema1[table]:
            if field not in schema2[table]:
                differences.append(f"Field {field} in table {table} is missing in the second schema.")
                continue
            if schema1[table][field] != schema2[table][field]:
                differences.append(f"Different definitions for field {field} in table {table}.")
    return differences
